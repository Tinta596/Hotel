...
export default {
  setup() {
    ...
    const cerrarModalPrecio = () => {
      modalPrecioVisible.value = false;
    };

    const actualizarPrecio = async () => {
      try {
        await api.put(`/habitaciones/${habitacionEditada.value.id}/precio`, {
          precio_base: habitacionEditada.value.precio
        });
    ...
    const guardarServicio = async () => {
      ...
      Object.entries(servicio.value).forEach(([k, v]) => {
        form.append(k, v);
      });
      if (imagen.value) {
    ...
    const toggleServicioActivo = async servicioItem => {
      try {
        ...
        toast.success(servicioItem.activo ? 'Servicio ocultado' : 'Servicio activado');
        cargarDatos();
      } catch (error) {
